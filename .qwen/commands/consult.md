...
Lấy **phản biện từ nhiều hãng AI khác nhau** cho quyết định/câu hỏi: `{{args}}`.
...
2. **Gọi** (script nằm chung tại `.claude/scripts/consult-litellm.py` — một script cho cả hai CLI):
   ```bash
   # cả 3 hãng, chạy song song (~30 giây)
   python3 .claude/scripts/consult-litellm.py "<bối cảnh gọn> | Câu hỏi: <{{args}}>. Phản biện: rủi ro, điểm mù, phương án khác."
