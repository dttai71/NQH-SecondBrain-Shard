...
5. Cập nhật xong, so sánh `TRUOC` với `HEAD` mới (nếu có git): liệt kê skill/command mới hoặc đổi — nhìn vào các đường dẫn `09 - Skills/`, `.claude/commands/`, `.claude/skills/`, `.qwen/commands/`, `.qwen/skills/` xuất hiện trong `git diff --stat TRUOC HEAD` (vault có cả 2 bộ, dù bạn đang chạy CC hay Qwen — báo đủ cả hai, không chỉ bộ của CLI đang chạy). Không có git (vault tải tay, không clone) → chỉ báo "đã cập nhật xong", không so sánh được.
